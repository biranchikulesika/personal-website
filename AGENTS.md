...
- **Automated Tests**: `npm test` _(runs native test suite via `npx tsx --test tests/_.test.ts`)\*
- **Node Version**: `>=24.16.0` _(enforced via `.nvmrc`: `24.18.0`)_
...
- Only when the repository owner gives explicit instruction (e.g., _"commit these changes"_, _"commit and push"_), the agent will stage, commit with a concise message, and/or push to remote.
