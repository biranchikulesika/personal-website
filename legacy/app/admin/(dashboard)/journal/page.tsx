...
export default function JournalMomentPage() {
  ...
  const [formData, setFormData] = useState<any>({"title":"","body":"","timeLabel":"","hidden": true});

  const loadData = async () => {
    const data = await getJournalMoments();
    setItems(data || []);
  };
  ...
  const handleCreateNew = () => {
    setEditingId(null);
    setFormData({"title":"","body":"","timeLabel":"","hidden": true});
    setIsEditing(true);
  };
  ...
  const handleSave = async () => {
    if (editingId && editingId !== 'none') {
      await updateJournalMoment(editingId, formData);
    } else {
      await createJournalMoment(formData);
    }
    setIsEditing(false);
    loadData();
  };
  ...
  const toggleVisibility = async (item: any) => {
    ...
    if (isCurrentlyHidden) {
      await unhideJournalMoment(item.id || item.persona);
    } else {
      await hideJournalMoment(item.id || item.persona);
    }
    loadData();
  };
  ...
  return (
    <div className="w-full max-w-350 mx-auto p-5 md:p-8 lg:p-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          <h1 className="text-3xl font-medium tracking-tight text-foreground mb-2">Journal Entries</h1>
          <p className="text-muted-text text-sm">Manage journal entries for the site.</p>
        </div>
        <button
      ...
      {isEditing && (
        <div className="mt-12 border-t border-border pt-12">
          ...
          <div className="space-y-6 bg-surface p-6 rounded-lg border border-border">
              ...
              <div>
                <label className="block text-xs uppercase tracking-widest text-muted-text font-mono mb-2">Body</label>
                <textarea value={formData.body || ''} onChange={(e) => setFormData({...formData, body: e.target.value})} className="w-full h-32 bg-muted border border-border rounded-md px-4 py-2.5 text-sm text-foreground outline-none focus:border-border resize-none"></textarea>
              </div>
              <div>
                <label className="block text-xs uppercase tracking-widest text-muted-text font-mono mb-2">Time Label</label>
                <input type="text" value={formData.timeLabel || ''} onChange={(e) => setFormData({...formData, timeLabel: e.target.value})} className="w-full bg-muted border border-border rounded-md px-4 py-2.5 text-sm text-foreground outline-none focus:border-border" />
              </div>
              <div>
