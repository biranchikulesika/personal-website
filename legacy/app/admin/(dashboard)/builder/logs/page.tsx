...
export default function BuildLogPage() {
  ...
  const [formData, setFormData] = useState<any>({"title":"","description":"","date":"","source":"manual","aiGenerated":false,"relatedCommits":[],"relatedRepositories":[],"hidden":false});
  ...
  const handleCreateNew = () => {
    setEditingId(null);
    setFormData({"title":"","description":"","date":"","source":"manual"});
    setIsEditing(true);
  };
  ...
  const handleSave = async () => {
    if (editingId && editingId !== 'none') {
      await updateBuildLog(editingId, formData);
    } else {
      await createBuildLog(formData);
    }
    setIsEditing(false);
    loadData();
  };
  ...
  return (
    <div className="w-full max-w-350 mx-auto p-5 md:p-8 lg:p-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          <h1 className="text-3xl font-medium tracking-tight text-neutral-100 mb-2">Build Logs</h1>
          <p className="text-neutral-500 text-sm">Manage build logs for the site.</p>
        </div>
        <button
      ...
      {isEditing && (
        <div className="mt-12 border-t border-[#1a1a1a] pt-12">
          ...
          <div className="space-y-6 bg-[#111111] p-6 rounded-lg border border-[#1a1a1a]">
              ...
              <div>
                <label className="block text-xs uppercase tracking-widest text-neutral-500 font-mono mb-2">Date</label>
                <input type="text" value={formData.date || ''} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full bg-[#161616] border border-[#222] rounded-md px-4 py-2.5 text-sm text-neutral-200 outline-none focus:border-neutral-500" />
              </div>
              <div>
