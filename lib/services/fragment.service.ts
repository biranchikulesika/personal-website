...
export class FragmentService {
  private repository: IRepository<Fragment>;

  constructor() {
    this.repository = repositoryRegistry.getFragmentRepository();
  }
  ...
  async hide(id: string): Promise<Fragment | null> {
    try {
      return await this.repository.update(id, { hidden: true } as Partial<Fragment>);
    } catch (error) {
      console.error("Failed to hide fragment:", error);
      throw error;
    }
  }

  async unhide(id: string): Promise<Fragment | null> {
    try {
      return await this.repository.update(id, { hidden: false } as Partial<Fragment>);
    } catch (error) {
      console.error("Failed to unhide fragment:", error);
      throw error;
    }
  }
